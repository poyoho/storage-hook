...
export interface StorageAccessor<Options> {
  getItem<K extends string & keyof Options>(key: K): Promise<InferKeyType<Options[K]>>
  setItem<K extends string & keyof Options>(key: K, val: InferKeyType<Options[K]>)
  clear(): void
  removeItem(key: string & keyof Options): void
}

export function useStorage<Options extends OptionMap = OptionMap>(options: Options, storage: Storage): StorageAccessor<Options> {
  const result: StorageAccessor<Options> = {
    async getItem<K extends string & keyof Options>(key: K): Promise<InferKeyType<Options[K]>> {
      ...
      switch (constructor) {
        case Boolean:
          formatData = value === 'true' ? true : (value === 'false' ? false : Boolean(value))
          break
        case Number:
          formatData = Number(value)
          if (Number.isNaN(formatData)) {
            return createAsyncCommonError(ErrorCodes.STORAGE_UNEXPECTED_NUMBER, `(${key})`)
          }
          break
        case Date: // save number can accurate to the millisecond
          formatData = Number(value)
          if (Number.isNaN(formatData)) {
            return createAsyncCommonError(ErrorCodes.STORAGE_UNEXPECTED_DATE, `(${key})`)
          }
        ...
        case Object:
          try {
            ...
            return createAsyncCommonError(ErrorCodes.STORAGE_UNEXPECTED_OBJECT, `(${key})`)
          }
          break
        case Array:
          try {
