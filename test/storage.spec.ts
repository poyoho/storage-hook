...
describe('storage', () => {
  test('no support local storage', () => {
    ...
    try {
      ...
      expect(e.message).toContain(
        errorMessages[ErrorCodes.LOCALSTORAGE_NOTSUPPORT]
      )
  ...
  test('no support session storage', () => {
    ...
    try {
      ...
      expect(e.message).toContain(
        errorMessages[ErrorCodes.SESSIONSTORAGE_NOTSUPPORT]
      )
...
function testUnit(factory: Factory, storage: Storage) {
  describe(`${factory.name} simple`, () => {
    test('number', async () => {
      ...
      const val_reject = await instance
        .getItem('number')
        .catch((res: StorageError) => res.code)
    ...
    test('clear', async () => {
      ...
      const val_reject = await instance
        ...
        .catch((res: StorageError) => res.code)
      expect(val_reject).toBe(ErrorCodes.STORAGE_KEY_UNDEFINED)
      const val_reject2 = await instance
        .getItem('clear2')
        .catch((res: StorageError) => res.code)
  ...
  describe(`${factory.name} unexpected`, () => {
    test('number unexpected', async () => {
      ...
      const val_unexpected = await instance
        .getItem('number_unexpected')
        .catch((e: StorageError) => e.code)
    ...
    test('array unexpected', async () => {
      ...
      const val_unexpected = await instance
        .getItem('array_unexpected')
        .catch((e: StorageError) => e.code)
      expect(val_unexpected).toBe(ErrorCodes.STORAGE_UNEXPECTED_OBJECT)

      storage.setItem('array_unexpected', 'unexpected')
      const val_unexpected2 = await instance
        .getItem('array_unexpected')
        .catch((e: StorageError) => e.code)
